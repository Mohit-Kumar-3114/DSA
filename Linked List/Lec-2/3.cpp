// Inserting node in a doubly linked list
...
void insertAtStart(Node* &head, int element) {
   Node* temp=new Node(element,head,nullptr);
...
void insertAtEnd(Node* &head, int element) {
  ...
  Node* n= new Node (element,nullptr,nullptr);
  temp->next=n;
  n->back=temp;
}
...
void insertAtKthPosition(Node* &head,int element,int k){
    ...
    Node* n = new Node(element, temp->next, temp);
    if (temp->next) temp->next->back = n; 
    temp->next = n;
    }
}
